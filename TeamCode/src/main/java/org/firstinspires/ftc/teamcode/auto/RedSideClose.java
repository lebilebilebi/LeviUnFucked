...
public class RedSideClose extends OpMode {
    ...
    private final Pose scorePose = new Pose(60, 84, Math.toRadians(139)).mirror(); // Scoring pose
    private final Pose endPose = new Pose(13, 70, Math.toRadians(90)).mirror(); // Ending pose
    private final Pose turn = new Pose(58, 82, Math.toRadians(180)).mirror(); //Turn from preload shoot to face balls for first intake
    private final Pose intake1 = new Pose(26, 82, Math.toRadians(180)).mirror(); // Drive to first and intake first line (in this case, start intake when pathing to this pose)
    private final Pose intakeStart2 = new Pose(55, 60, Math.toRadians(180)).mirror(); // Drive to second line
    private final Pose intake2 = new Pose(26, 60, Math.toRadians(180)).mirror(); // Intake along second line
    private final Pose intakeStart3 = new Pose(55, 37, Math.toRadians(180)).mirror(); // Drive to third line
    private final Pose intake3 = new Pose(26, 37, Math.toRadians(180)).mirror(); // Intake along third line
